...
- **Live-smoke follow-up — final-review closure hardened**: a real two-step goal exposed a completion race where the `goal_final_review` task reached `Completed` with `result = null` before its final assistant `VERDICT: PASS` message was persisted; the daemon read the goal root thread, failed to parse a verdict, and queued another final review. Finalization now reads only the review task's own thread, waits a bounded 5×50 ms for a parseable terminal verdict, fails closed with an explicit protocol error if none appears, and tells reviewers not to query goal/task status through CLI/API or leave the result empty. Focused final-review suite: 6/6 pass; cargo check passes.
