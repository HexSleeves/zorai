...
fn slash_context_with_tokens_updates_active_svarog_thread() {
    ...
    assert_eq!(
        model.current_header_agent_profile().context_window_tokens,
        Some(200_000)
    );
...
#[test]
fn slash_context_opens_editor_for_weles_thread_owner() {
    let (mut model, _daemon_rx) = make_model();
    seed_active_weles_thread(&mut model);
    ...
    assert!(model
        .context_window_editor_body()
        .contains("Pinned to Weles"));
}

#[test]
fn slash_context_editor_enter_saves_thread_owner_window() {
    let (mut model, mut daemon_rx) = make_model();
    seed_active_weles_thread(&mut model);
    ...
    while let Ok(command) = daemon_rx.try_recv() {
        if matches!(
            ...
            DaemonCommand::SetTargetAgentContextWindow {
                target_agent_id,
                context_window_tokens,
            } if target_agent_id == "weles" && context_window_tokens == 180_000
        ) {
            saw_target_update = true;
        }
    }
